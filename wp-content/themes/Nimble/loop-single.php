<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
	...
	<?php
		...
		$width = apply_filters('et_blog_image_width',621);
		if ( 'on' == $et_full_post ) $width = apply_filters( 'et_single_fullwidth_image_width', 960 );
		$height = apply_filters('et_blog_image_height',320);
		if ( 'project' == $post_type ) $height = apply_filters('et_project_single_image_height',9999);
		$classtext = '';
	...
	<?php 
		if ( et_get_option('nimble_468_enable') == 'on' ){
			if ( et_get_option('nimble_468_adsense') <> '' ) echo( et_get_option('nimble_468_adsense') );
			   ...
			   <a href="<?php echo esc_url(et_get_option('nimble_468_url')); ?>"><img src="<?php echo esc_url(et_get_option('nimble_468_image')); ?>" alt="468 ad" class="foursixeight" /></a>
	<?php 	}    
		}
	?>
