...
<?php if ( 'false' == et_get_option( 'nimble_blog_style', 'false' ) ){ ?>
	...
	<?php if ( 'on' == et_get_option( 'nimble_display_fromblog_section', 'on' ) ){ ?>
		<div id="home-section-news" class="home-section">
			...
			<?php
				$i = 1;
				$blog_posts_per_row = apply_filters( 'et_blog_posts_per_row', 3 );
			?>
		...
		<div id="home-section-projects" class="home-section">
			...
			<?php
				$portfolio_args = array(
					'post_type' => 'project',
					'showposts' => (int) et_get_option( 'nimble_homepage_numposts_projects', '3' )
				);
				...
				if ( false != et_get_option( 'nimble_homepage_exlcats_projects', false ) )
					$portfolio_args['tax_query'] = array(
						array(
							...
							'field' => 'id',
							'terms' => (array) et_get_option( 'nimble_homepage_exlcats_projects' ),
							'operator' => 'NOT IN'
						)
					);
				...
				if ( $portfolio_query->have_posts() ){
					$i = 1;
					$portfolios_per_row = apply_filters( 'et_portfolios_per_row', 3 );
					
					echo '<div id="portfolio_items">';
						while ( $portfolio_query->have_posts() ) : $portfolio_query->the_post();			
							$thumb = '';
							$width = apply_filters( 'et_portfolio_image_width', 266 );
							$height = apply_filters( 'et_portfolio_image_height', 266 );
							$classtext = '';
							$titletext = get_the_title();
							...
							if ( '' != $thumb ) {
								echo '<div class="portfolio-image' . ( $i % $portfolios_per_row == 0 ? ' last' : '' ) . '">';								
									...
									echo '<div class="portfolio_description">';
										...
										echo '<a href="' . get_permalink() . '">' . __( 'Read More', 'Nimble' ) . '</a>';
									echo '</div>';
